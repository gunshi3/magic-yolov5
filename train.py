...
# from utils.myautoanchor import check_anchors
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    LOGGER.info(f"{colorstr('optimizer:')} {type(optimizer).__name__} with parameter groups "
                f"{len(g1)} weight, {len(g0)} weight (no decay), {len(g2)} bias")
    del g0, g1, g2

    # Scheduler
    if opt.linear_lr:
        lf = lambda x: (1 - x / (epochs - 1)) * (1.0 - hyp['lrf']) + hyp['lrf']  # linear 1->hyp['lrf']
    else:
